...
/**
 * 本地图片加载器,采用的是异步解析本地图片，单例模式利用getInstance()获取NativeImageLoader实例 调用loadNativeImage()方法加载本地图片，此类可作为一个加载本地图片的工具类
 * 
 */
public class OriginalImageLoader {
	private LruCache<String, Bitmap> mMemoryCache;
	private static OriginalImageLoader mInstance = new OriginalImageLoader();
	...
	/**
	 * 此方法来加载本地图片，这里的mPoint是用来封装ImageView的宽和高，我们会根据ImageView控件的大小来裁剪Bitmap 如果你不想裁剪图片，调用loadNativeImage(final String
	 * path, final NativeImageCallBack mCallBack)来加载
	 * 
	 * @param path
	 * @param mPoint
	 * @param mCallBack
		...
		if (bitmap == null) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					Bitmap mBitmap = BitmapUtils.decodeThumbBitmapForFile(path, mPoint == null ? 0 : mPoint.x,
