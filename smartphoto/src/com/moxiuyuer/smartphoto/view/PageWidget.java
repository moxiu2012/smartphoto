...
public class PageWidget extends FrameLayout {
	 ...
	 * @param ev 事件
	 */
	public boolean autoTouch(MotionEvent event) {
...
//		System.out.println("========drawChild=======");
	...
	private void drawCurrentPageArea(Canvas canvas, View child, Path path) {
		//当翻页接近中线时，处理一些特殊值
		if ((mCornerX == 0 && mBeziervertex1.x > mWidth / 2) || (mCornerX == mWidth && mBeziervertex1.x < mWidth / 2)) {
			mBezierControl1.x = mWidth / 2;
			mBezierStart1.x = mWidth / 2;
		}
		if ((mCornerX == 0 && mBeziervertex2.x > mWidth / 2) || (mCornerX == mWidth && mBeziervertex2.x < mWidth / 2)) {
			mBezierControl2.x = mWidth / 2;
			mBezierStart2.x = mWidth / 2;
		}
	...
	private void drawNextPageAreaAndShadow(Canvas canvas, View child) {
		...
		//当翻页接近中线时，处理一些特殊值
		if ((mCornerX == 0 && mBeziervertex1.x > mWidth / 2) || (mCornerX == mWidth && mBeziervertex1.x < mWidth / 2)) {
			mBeziervertex1.x = mWidth / 2;
			mBezierStart1.x = mWidth / 2;
		}
		if ((mCornerX == 0 && mBeziervertex2.x > mWidth / 2) || (mCornerX == mWidth && mBeziervertex2.x < mWidth / 2)) {
			mBeziervertex2.x = mWidth / 2;
			mBezierStart2.x = mWidth / 2;
		}
		...
		// 在中线处，阴影范围减小，减小阴影在中线出的范围
		float rate = 4.0f;
		if ((mWidth - mTouchToCornerDis) / mWidth < 0.2) {
			rate = 1 / ((10 + mWidth - mTouchToCornerDis) / mWidth);
		}

		if (mIsRTandLB) {
			leftx = (int) (mBezierStart1.x);
			rightx = (int) (mBezierStart1.x + mTouchToCornerDis / rate);
			mBackShadowDrawable = mBackShadowDrawableLR;
		} else {
			leftx = (int) (mBezierStart1.x - mTouchToCornerDis / rate);
		...
		//限制阴影区在中心时的范围，小于一定值时，隐藏
		if(rate <30) {
			mBackShadowDrawable.setBounds(leftx, (int) mBezierStart1.y, rightx, (int) (mMaxLength + mBezierStart1.y));
		} else {
			mBackShadowDrawable.setBounds(mWidth, (int) mBezierStart1.y, mWidth, (int) (mMaxLength + mBezierStart1.y));
	...
	private void createDrawable() {
		...
		mBackShadowColors = new int[] { 0x50111111, 0x111111 };
		...
		mFrontShadowColors = new int[] { 0x50111111, 0x111111 };
	...
	private void drawCurrentBackArea(Canvas canvas, View view) {
		...
		/*
		 * int i1 = (int) (mBezierStart2.y + mBezierControl2.y) / 2; float f2 = Math.abs(i1 - mBezierControl2.y);
		 */
		float f3 = f1;
		mPath1.reset();
		...
		mMatrix.reset();
		mMatrix.setPolyToPoly(new float[] { Math.abs(mWidth - mCornerX), mCornerY }, 0, new float[] { mTouch.x,
				mTouch.y }, 0, 1);
